...
BEGIN
    ...
    INNER JOIN ESTADO_SOLICITUD ES ON S.ESTADO_SOLICITUD = ES.ID
    INNER JOIN TIPO_SOLICITUD TS ON S.TIPO_SOLICITUD = TS.ID
    WHERE S.TIPO_SOLICITUD = idTipo AND S.ID_USUARIO = idPersona;
END //
DELIMITER ;
...
INNER JOIN 
    PERSONA ON USUARIO.ID_PERSONA = PERSONA.ID WHERE NOTICIA.INHABILITADA = 0;
END //
DELIMITER ;
...
BEGIN
    SELECT * FROM NOTICIA WHERE ID = n_id AND INHABILITADA = 0;
END //
DELIMITER ;
