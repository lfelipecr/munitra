function EnviarCaptcha() {
  ...
  $("#captchaInput").val("");
  $("#catpchaInfo").html(
    "Ingrese el siguiente código para enviar su consulta:"
  );
...
$(document).ready(function () {
  ...
  $("#btnActualizar").on("click", function () {
    if ($("#cuerpoConsulta").val().trim() != "") {
      ...
      $.ajax({
        ...
        type: "POST",
        data: { idConsulta: idConsulta, consulta: cuerpo },
  ...
  $("#btnEnviarCorreo").on("click", function () {
    ...
    if (captcha == captchaGenerado) {
      ...
      if (
        $("#identificacionConsulta").val().trim() != "" &&
        $("#nombreConsulta").val().trim() != "" &&
        $("#telefonoConsulta").val().trim() != "" &&
        $("#correoConsulta").val().trim() != "" &&
        $("#asuntoConsulta").val().trim() != "" &&
        $("#cuerpoConsulta").val().trim() != ""
      ) {
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            if (response == "0") {
              $("#catpchaInfo").html(
                "Si tiene una solicitud registrada, no puede generar una nueva!"
              );
              ...
              $("#infoModal").html(
                "<span>Su solicitud ha sido enviada y su código de solicitud es: " +
                  response +
                  ".<br>La respuesta a su consulta llegará a su whatsapp o correo electrónico, agradecemos su paciencia</span>"
              );
            }
          },
          error: function (xhr, status, error) {
      ...
      intentos++;
      $("#catpchaInfo").html(
        "Su código no es correcto, ingrese este nuevo código:"
      );
      captchaGenerado = Math.random()
        .toString(36)
        .substring(2, 8)
        .toUpperCase();
      captcha = captchaGenerado;
