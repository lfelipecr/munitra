$(document).ready(function (){
    ...
    function RenderizarTabla() {
        ...
        for(let i = 0; i < datos.length; i++){
            tabla += `<tr>
                <td>${datos[i][0]}</td>
                <td>${datos[i][6]} ${datos[i][9]}  ${datos[i][10]}</td>
                <td>${datos[i][8]}</td>
                <td>${datos[i][22]}</td>
                <td class="align-items-center d-flex justify-content-end">
                    <a class="btn btn-warning mx-1" href="index.php?controlador=Usuario&metodo=VActualizar&id=${datos[i][0]}">
    ...
    $('#txtBusqueda').on('keyup', function () {
        ...
        if (query == '' || query == null){
            ...
            let tabla = "";
            for(let i = 0; i < datos.length; i++){
                if (datos[i][0].includes(query) || datos[i][6].includes(query) || 
                    datos[i][8].includes(query) || datos[i][22].includes(query)){
                    tabla += `<tr>
                        <td>${datos[i][0]}</td>
                        <td>${datos[i][6]} ${datos[i][9]}  ${datos[i][10]}</td>
                        <td>${datos[i][8]}</td>
