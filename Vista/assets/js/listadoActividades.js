$(document).ready(function () {
  ...
  $("#txtBusqueda").on("keyup", function () {
    ...
    if (query == "" || query == null) {
      ...
      datos.forEach((actividad) => {
        if (
          actividad.id.includes(query) ||
          actividad.titulo.includes(query) ||
          actividad.idUsuario.includes(query) ||
          actividad.fecha.includes(query)
        ) {
          tabla += `<tr>
                        <td>${actividad.id}</td>
                        <td>${actividad.titulo}</td>
          ...
          if (idUsuario == actividad.idUsuario) {
            tabla += `<td class="align-items-center d-flex justify-content-end">
                                    <a class="btn btn-danger mx-1" href="index.php?controlador=Actividad&metodo=Eliminar&id=${actividad.id}">
